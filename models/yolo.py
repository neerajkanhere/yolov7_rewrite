...
from models.config_builder import create_model_from_config
from loss import PredIdx, transform_model_outputs_into_predictions
...
class Yolov7Model(nn.Module):
    ...
    def postprocess(
        ...
        formatted_preds = self._format_preds(
            preds, conf_thres, nms_thres, max_detections, multiple_labels_per_box
        )
    ...
    def _derive_preds(self, fpn_heads_outputs):
        ...
        for layer_idx, fpn_head_outputs in enumerate(
            ...
            ] *= self.detection_head.strides[layer_idx]  # -> Image coordinates
    ...
    def _format_preds(
        ...
        for image_idx, detections_for_image in enumerate(
            ...
            nms_idx = torchvision.ops.batched_nms(
                boxes=detections_for_image[:, :4],
                scores=detections_for_image[:, 4],
                idxs=detections_for_image[:, 5],
                iou_threshold=nms_thres,
            )
            formatted_preds[image_idx] = detections_for_image[nms_idx]
