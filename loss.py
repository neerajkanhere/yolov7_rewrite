...
def _box_xyxy_to_cxcywh(boxes):
    ...
    """
    image_id, label_id, x1, y1, x2, y2 = boxes.unbind(-1)
    cx = (x1 + x2) / 2
    cy = (y1 + y2) / 2
    w = x2 - x1
    h = y2 - y1

    boxes = torch.stack((image_id, label_id, cx, cy, w, h), dim=-1)
...
class Yolov7Loss:
    ...
    def __call__(
        ...
        # print(targets.shape)
        targets = _box_xyxy_to_cxcywh(targets)
